...
相较optic v4.1更新：
1. 用两个进程分别制作Mask和补洞，总时长396s，平均每帧0.82s，效率为v4.1的1.875倍
...
def makeVideo(all_inpaints, filename, num):
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter(filename + '.mp4',fourcc, 20.0, (1436,664))

    for _ in range(num):
        inpaint = all_inpaints.get()
        out.write(inpaint)
        cv2.imshow('img', inpaint)
...
# %%
def makeMask(all_masks, frames, vertex, first_frame, last_frame, strides, do_plot = 1, mode = 'run'):
...
# %%
def paint(all_inpaints, all_masks, all_frames, vertex, first_frame, last_frame, stride, do_plot = 1, mode = 'run'):
    frames = []
    masks = []
    ...
    for i in np.arange(first_frame, last_frame - stride + 1, 1):
        ...
        if i + source[-1] <= last_frame - stride:
            masks.append(all_masks.get())
        else:
            masks.append(None)
...
if __name__ == "__main__":
    ...
    all_masks = Queue()
    all_inpaints = Queue()
    pw = Process(target = makeMask, args = (all_masks, frames, vertex, first_frame, last_frame, strides, 0, 'run'))
    pr = Process(target = paint, args = (all_inpaints, all_masks, frames, vertex, first_frame, last_frame, stride_max, 0, 'run'))
    pv = Process(target = makeVideo, args = (all_inpaints, 'Video', last_frame - first_frame - stride_max + 1))
    pw.start()
    pr.start()
    pv.start()
    pr.join()
