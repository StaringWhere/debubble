...
# %%
def makeMask(all_masks, frames, vertex, frame_id, strides, do_plot = 1):
    ...
    for stride in strides:
        frame2 = frames[frame_id + stride - 1]
        if frame2 is None:
            break
        gray2 = cv2.cvtColor(frame2, cv2.COLOR_BGR2GRAY)

        #--------------------恢复背景位移-------------------
        gray2_shift, mag_bg = alignImg(vertex, gray, gray2, frame_id, frame_id + stride)
    ...
    all_masks[frame_id - 1] = mask

def makeMasks(all_masks, frames, vertex, first_frame, last_frame, strides, proc_num, proc_id, do_plot = 1):
    for i in np.arange(first_frame + proc_id, last_frame - max(strides) + 1, proc_num):
        makeMask(all_masks, frames, vertex, i, strides, do_plot)
# %%
def paint(all_inpaints, all_masks, all_frames, vertex, frame_id, stride_max, source, do_plot = 1):
    # 准备要用到的数据
    frames = []
    masks = []
    # 待修复的帧在数组中的位置
    frame_index = list(source).index(0)

    #---------------------读取---------------------
    for j in source:
        try:
            frames.append(all_frames[frame_id + j - 1])
            masks.append(all_masks[frame_id + j - 1])
        except IndexError:
            frames.append(None)
            masks.append(None)
    ...
    for j in source[np.argsort(np.abs(source))]:
        ...
        # 运动信息
        frame2_shift, _ = alignImg(vertex, frames[frame_index], frames[frame_index + j], frame_id, frame_id + j, fill = 0)
        mask2_shift, _ = alignImg(vertex, masks[frame_index], masks[frame_index + j], frame_id, frame_id + j, fill = 0)
    ...
    #------------------------保存-----------------------------
    # cv2.imwrite('dense/inpaint_dense/inpaint_{}.jpg'.format(frame_id), inpaint)
    ...
    all_inpaints[frame_id - 1] = inpaint

def paints(all_inpaints, all_masks, all_frames, vertex, first_frame, last_frame, stride_max, proc_num, proc_id, do_plot = 1):
    # 从其他图片补第0张图片，必须为间距是1的等差数列
    source = np.arange(-10, 11, 1)

    for i in np.arange(first_frame + proc_id, last_frame - stride_max + 1, proc_num):
        # 判断要用到的mask的范围下界
        if i + source[0] < first_frame:
            start_frame = first_frame
        else:
            start_frame = i + source[0]

        # 判断要用到的mask的范围上界
        if i + source[-1] > last_frame - stride_max:
            end_frame = last_frame - stride_max
        else:
            end_frame = i + source[-1]

        while True in [mask is None for mask in all_masks[start_frame - 1: end_frame]]:
            sleep(0.1)

        paint(all_inpaints, all_masks, all_frames, vertex, i, stride_max, source, do_plot)

        print("proc: ", proc_id, "frame: ", i, "inpaint finished")
...
if __name__ == "__main__":
    ...
    # Make Mask
    # all_masks在共享内存中
    all_masks = manager.list([None] * len(frames))
    for proc_id in range(proc_num1):
        p = Process(target = makeMasks, args = (all_masks, frames, vertex, first_frame, last_frame, strides, proc_num1, proc_id, 0))
        processes.append(p)
        p.start()

    # Inpaint
    # all_inpaints在共享内存中
    all_inpaints = manager.list([None] * len(frames))
    for proc_id in range(proc_num2):
        p = Process(target = paints, args = (all_inpaints, all_masks, frames, vertex, first_frame, last_frame, stride_max, proc_num2, proc_id, 0))
        processes.append(p)
        p.start()
